...
Set-ProfileAlias -Name mkdir -FunctionName New-VerboseDirectory -ScriptBlock {
    foreach ($Path in $args) {
        New-Item -ItemType Directory -Force -Verbose -Path $Path
    }
}

Set-ProfileAlias -Name cd -FunctionName Set-LocationAndList -ScriptBlock {
    if ($args.Count -eq 0) {
...
if (Get-Command nixos-rebuild -ErrorAction SilentlyContinue) {
    Set-ProfileAlias -Name nix-up -FunctionName Invoke-NixOsRebuild -ScriptBlock {
        sudo nixos-rebuild switch --flake ~/dotfiles#nixos @args
    }
}
